...
class ProfessionalUserController {
   ...
   static async getById(req, res) {
        const { _id } = req.body;
        try {
            const professionalUser = await ProfessionalUserModel.findById(_id);
            if (!professionalUser) {
   ...
   static async loginUser(req, res) {
    ...
    static async update(req, res) {
        const { _id } = req.body;
        try {
            let professionalUser = await ProfessionalUserModel.findById(_id);
            if (!professionalUser) {
                return res.status(404).json({ error: 'Professional user not found' });
            }
    ...
    static async delete(req, res) {
        const { _id } = req.body;
        try {
            const deletedProfessionalUser = await ProfessionalUserModel.findByIdAndDelete(_id);
